...
var DB = openDatabase('pages', '1.0', 'Monitored Pages', 100 * 1024 * 1024);
...
var DATABASE_STRUCTURE = "CREATE TABLE IF NOT EXISTS pages ( \
  ...
  `initial_price` TEXT, \
  `initial_price_date` INTEGER, \
  `current_price` TEXT, \
  `min_price` TEXT, \
  `min_price_date` INTEGER, \
  `order_number` INTEGER, \
  `folder_name` TEXT \
);";

var DATABASE_TABLE_LINKS ="CREATE TABLE IF NOT EXISTS links( \
 `url` TEXT NOT NULL UNIQUE, \
 `order_number` INTEGER \
  XX);";
...
function initializeStorage(callback) {
  executeSql(DATABASE_STRUCTURE, $.noop, function(){
    executeSql(DATABASE_TABLE_LINKS, $.noop, callback);
...
function getAllPageURLs(callback) {
  ...
  executeSql('SELECT url FROM pages ORDER BY order_number', [], function(result) {
    var urls = [];
    for (var i = 0; i < result.rows.length; i++) {
...
function getAllPages(callback) {
  ...
  executeSql('SELECT * FROM pages ORDER BY order_number', [], function(result) {
...
function addPage(page, callback) {
  ...
  page.url = removeTags(page.url);

  var query = "REPLACE INTO pages(url, name, mode, regex, selector, \
                                  check_interval, html, crc, updated, \
                                  last_check, last_changed, initial_price, initial_price_date, \
                                  current_price, min_price, min_price_date, order_number, folder_name ) \
               VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  executeSql(query, [
    ...
    page.initial_price || null,
    page.initial_price_date || null,
    page.current_price || null,
    page.min_price || null,
    page.min_price_date || null,
    page.order_number || null,
    page.folder_name || null
  ], null, function() {
    BG.takeSnapshot();
    BG.scheduleCheck();
    (callback || $.noop)();
...
function removeTags(url)
{
   if (url.indexOf("amazon.com")>0)
   {
      if (url.indexOf("&tag=spmtrc-20")>0)
     {  
        return url.replace("&tag=spmtrc-20","");
     }
  }
  else if (url.indexOf("booking.com") > 0)
  {   
    if (url.indexOf("aid=859976;") > 0)
    {
      return url.replace("aid=859976;","");
    }
    else if (url.indexOf("aid=859976") > 0)
    {
      return url.replace("aid=859976","");
    }
  }
  ...
  getPage(url, function(page) {
    ...
    $.ajax({
      ...
      success: function(html, _, xhr) {
        ...
        findAndFormatSelectorMatches(html, page.selector, function(htmlselector){
        ...
        cleanAndHashPage(html, page.mode, page.regex, page.selector,
          ...
          if ((crc != page.crc && checkPriceValid(htmlselector) != checkPriceValid(page.current_price) && checkPriceValid(htmlselector) != "") || 
                    ((page.min_price == null ||  page.min_price == "") && checkPriceValid(htmlselector) != "")) {
            ...
            settings = {
              ...
              current_price: htmlselector,
              min_price: get_min(htmlselector, page.min_price),
              min_price_date:(get_min(htmlselector, page.min_price) != htmlselector) ? Date.now() : page.min_price_date
            }
...
function get_min(current, min)
{
  if (current == null)
    return null;
  
  var currentNum = + current.match(/\d/g).join("");
  
  if (min == null)
    return current;
  
  var minNum = +min.match(/\d/g).join("");
