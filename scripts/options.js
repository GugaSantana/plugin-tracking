...
var MIN_TIME_TEXTBOX_VALUE = 10;  // ~5 seconds.
...
function setPageRegexOrSelector(url, mode, value) {
 ...
 $.ajax({
    ...
    findAndFormatSelectorMatches(html, value, function(htmlselector){
    ...
    if (value === null)  {
      ...
      if (valid) {
        ...
        if (is_regex) {
          ...
          settings.initial_price = htmlselector;
          settings.initial_price_date = Date.now();
          settings.current_price = htmlselector;
          settings.min_price = htmlselector;
          settings.min_price_date = Date.now();
        }
  ...
  getAllPages(function(pages) {
    ...
    for (var i in pages) {
      ...
      var encoded_settings = JSON.stringify({
        ...
        initial_price: pages[i].initial_price,
        initial_price_date: pages[i].initial_price_date,
        min_price: pages[i].min_price,
        min_price_date: pages[i].min_price_date,
        order_number: pages[i].order_number,
        folder_name: pages[i].folder_name
...
function initializeIntervalSliders() {
  ...
  textbox.val(4).change(function() {
    var val_ms = parseFloat($(this).val()) * 60 * 1000;
    if (val_ms < 30000) val_ms = 30000;
...
function initializePageAdvancedToggler() {
  $('.advanced_toggle input[type=checkbox]').live('click', PageAdvancedTogglerClickHandler);
...
function PageAdvancedTogglerClickHandler()
{
    var url = findUrl(this);
    ...
    if ($(this).is(':checked')) {
      ...
      if ($(':checked', interval_div).length > 0) {
        var interval_log = $('input[type=text]', interval_div).val();
...
function selectorServer(request, _, callback) {
  if (request && request.selector) {
    ...
    if (page_record.length > 0 )
     // PageAdvancedTogglerClickHandler();
      
      $('.advanced_toggle input', page_record).attr({ checked: true });
      $('.advanced_controls', page_record).css({ display: 'block' });
      $('.mode input', page_record).attr({ checked: true });
      setPageRegexOrSelector(request.url, 'selector', request.selector);
